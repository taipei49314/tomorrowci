...
| Post-seal public replay receipt persistence | OPEN | current v2 `replay` implementation | command executes from verified evidence, but does not append a receipt or modify the sealed run | Phase 1 is not complete; follow-up evidence API/workflow required |
